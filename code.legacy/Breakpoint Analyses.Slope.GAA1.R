...
dt. %<>% 
  select( sjid, status, fxn, sex, aoo, gaa100, pm, group )

dt. <- readRDS('../R.RCR.Modeling/DATA derived/fars.slopes.rds') %>% filter(study == 'ALLFARS') %>% 
  left_join(
    dt.
  ) %>% 
  filter(!is.na(status))

group1 <- levels(factor(fxn.$group))[3]
...
dt. %>% ggplot(aes(x=bl, y = slope))+geom_point()
...
find_optimal_breakpoint <- function(data) {
  ...
  fit_broken_stick <- function(data, breakpoint) {
    ...
    model <- lm(slope ~ gaa100 + gaa100_above_bp, data = data)
...
correlation_data <- dt_modified %>%
  group_by(group) %>%
  summarise(
    below_corr = cor(gaa100[gaa100_below_threshold], slope[gaa100_below_threshold], method = "spearman", use = "pairwise.complete.obs"),
    above_corr = cor(gaa100[!gaa100_below_threshold], slope[!gaa100_below_threshold], method = "spearman", use = "pairwise.complete.obs"),
    overall_corr = cor(gaa100, slope, method = "spearman", use = "pairwise.complete.obs")  # Overall correlation
  )
...
ggplot(dt_modified, aes(x = gaa100, y = slope)) +
  geom_point() +
  ...
  labs(title = "Broken Stick Models with Correlations", x = "GAA100", y = "slope") +
  theme_minimal() +
  # aes(color = factor(obs))+
  aes(color = factor(pm))+
