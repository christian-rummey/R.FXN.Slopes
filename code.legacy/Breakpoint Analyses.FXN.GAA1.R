...
find_optimal_breakpoint <- function(data) {
  ...
  fit_broken_stick <- function(data, breakpoint) {
    ...
    model <- lm(fxn ~ gaa100 + gaa100_above_bp, data = data)
...
correlation_data <- dt_modified %>%
  group_by(group) %>%
  summarise(
    below_corr = cor(gaa100[gaa100_below_threshold], fxn[gaa100_below_threshold], method = "spearman", use = "pairwise.complete.obs"),
    above_corr = cor(gaa100[!gaa100_below_threshold], fxn[!gaa100_below_threshold], method = "spearman", use = "pairwise.complete.obs"),
    overall_corr = cor(gaa100, fxn, method = "spearman", use = "pairwise.complete.obs")  # Overall correlation
  )
...
ggplot(dt_modified, aes(x = gaa100, y = fxn)) +
  geom_point() +
  ...
  labs(title = "Broken Stick Models with Correlations", x = "GAA100", y = "fxn") +
  theme_minimal() +
