...
source('DM FXN Slope models DM.R')
...
fxn.dt. <- dt. %>% 
  select( study, sjid, sev.o, gaa100, time., paramcd, bl, aval, phase.n ) %>% 
  left_join(
    fxn.tmp %>% 
      select( sjid, analysis.group, type, value ),
    relationship = "many-to-many"
    ) %>%
  filter(!is.na(analysis.group)) %>% 
  group_by(analysis.group, paramcd)

table(fxn.$analysis.group)

# fxn.dt. <- fxn. %>% 
#   left_join(dt.) %>% filter(!is.na(paramcd)) %>% 
#   unnest(data) %>% group_by(group)

table. <- fxn.dt. %>% 
  select ( analysis.group, sjid, avisitn, paramcd, bl  )
  # filter( bl < 30) %>% 
  nest() %>% 
